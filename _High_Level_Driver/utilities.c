...
{    
    if(mTickCompare(var->tick) >= var->period)
    {
        ...
        var->buffer.p[var->index_buffer] = (float) ADC10Read(var->adc_module);
        var->sum_of_buffer += var->buffer.p[var->index_buffer];
...
bool fu_ntc(NTC_VAR *var)
{
    ...
    if (ret)
    {
        var->temperature = (float) (((1.0/((1.0/(var->ntc_params.t0+273.15)) + ((1.0/var->ntc_params.b) * log(((float)(10000.0/((1024/(var->average.sum_of_buffer/var->average.buffer.size)) - 1.0)))/var->ntc_params.r0 )))) - 273.15) + 40.0); // °C
    }
    return ret;
}
...
/*******************************************************************************
 * Function: 
 *      uint8_t fu_get_decimal_value(float v)
 * 
 * Description:
...
void background_tasks(ACQUISITIONS_VAR *var)
{
    ...
    switch(mux_sel)
    {
        ...
        case 1:     // CURRENT
            ...
            {
                var->current.average = (float) (var->current.average * 5.04 / 100000.0);    // ((((_ucAcquisitions.tabSumCurrent / 10) * 3.3) / 1023.0) / 6.4);
        ...
        case 2:     // VOLTAGE
            ...
            {
                var->voltage.average = (float) (var->voltage.average * 1.84 / 1000.0);    // ((((tabSumVoltage / 10) * 3.3) / 1023.0) / 0.1754);
