...
/*******************************************************************************
  ...
  Description:
    This routine set the desire idle state on the Tx pin.

  Parameters:
    mUartModule             - Identifies the desired UART module.
    idleState               - The desired idle state.

  Returns:

  Example:
    <code>
    UARTIdleStateTx(UART1A, 0);    // Idle state '0' (default, idle state = '1')
    </code>

  *****************************************************************************/
void UARTIdleStateTx(UART_MODULE mUartModule, BOOL idleState)
{
    UART_REGS * uartRegister = (UART_REGS *)uartReg[mUartModule];

    if(!idleState)
    {
        uartRegister->sta.set = UART_INVERT_TRANSMIT_POLARITY >> 16;
    }
    else
    {
        uartRegister->sta.clr = UART_INVERT_TRANSMIT_POLARITY >> 16;
    }
}
...
/*******************************************************************************
  ...
  Description:
    This routine set the desire idle state on the Rx pin.
  ...
  Example:
    <code>
    UARTIdleStateRx(UART1A, 0);    // Idle state '0' (default, idle state = '1')
    </code>
...
void UARTIdleStateRx(UART_MODULE mUartModule, BOOL idleState)
{
    ...
    if(!idleState)
    {
        uartRegister->sta.set = UART_INVERT_RECEIVE_POLARITY;
    }
    else
    {
        uartRegister->sta.clr = UART_INVERT_RECEIVE_POLARITY;
    }
}
