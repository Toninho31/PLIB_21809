    ...
    switch(discoverySM)
    {
        ...
        case DISCOVERY_REPLY:
            ...
            if(UDPIsPutReady(socket))
            {
                ...
                UDPPut(fu_get_decimal_value(acquisitions.ntc.temperature));
#if (DISCOVERY_VERSION >= 4)
                /* UC Voltage */
                UDPPut(fu_get_integer_value(acquisitions.voltage.average));
                UDPPut(fu_get_decimal_value(acquisitions.voltage.average));
                /* UC Current */
                UDPPut(fu_get_integer_value(acquisitions.current.average));
                UDPPut(fu_get_decimal_value(acquisitions.current.average));
#endif
#endif
#endif
                /* Send packet */
